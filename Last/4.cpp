...
template <typename T>
SmartPtr<T>::SmartPtr(T* pValue) : m_pData(pValue) {
    m_pReference = new KRefCount();
    m_pReference->AddRef();
}

template <typename T>
SmartPtr<T>::SmartPtr(const SmartPtr<T>& sp) : m_pData(sp.m_pData), m_pReference(sp.m_pReference) {
    m_pReference->AddRef();
}
...
SmartPtr<T>& SmartPtr<T>::operator=(T* pValue) {
    if (m_pData != pValue) {
        KRefCount* pNewRef = new KRefCount();
        pNewRef->AddRef();
        if (m_pReference->Release() == 0) {
            delete m_pData;
            delete m_pReference;
