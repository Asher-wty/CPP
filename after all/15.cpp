...
public:
    ...
    // 新增：截取子向量 [first, last)
    DynamicVector operator()(int first, int last) const;
    // 新增：交换两个向量
...
void DynamicVector<T>::remove(int Vindex)
{
    ...
    for (unsigned i = pos; i < numofItems - 1; ++i) {
        array[i] = array[i + 1];
    }
    --numofItems;
}
...
{
    int first = Vfirst - virtualZero;
    int last = Vlast - virtualZero;
    if (first < 0 || first > (int)numofItems || last < first || last > (int)numofItems) {
        cout << endl << "Out Of Range";
        exit(1);
    }
    int count = last - first;
    for (int i = first; i < (int)numofItems - count; ++i) {
...
template <class T>
DynamicVector<T> DynamicVector<T>::operator()(int first, int last) const
{
    if (first < virtualZero || last > virtualZero + (int)numofItems || first > last) {
        cout << endl << "Out Of Range";
        exit(1);
    }
    DynamicVector<T> result(first);
...
template <class T>
void DynamicVector<T>::swap(DynamicVector& other)
{
    T* tmpArray = array;
    array = other.array;
    other.array = tmpArray;

    unsigned tmpSize = mallocSize;
    mallocSize = other.mallocSize;
    other.mallocSize = tmpSize;

    unsigned tmpNum = numofItems;
    numofItems = other.numofItems;
    other.numofItems = tmpNum;

    int tmpZero = virtualZero;
    virtualZero = other.virtualZero;
    other.virtualZero = tmpZero;
}
