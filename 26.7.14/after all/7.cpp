...
HugeInt HugeInt::operator*( const HugeInt & other ) const
{
    ...
    for ( int i = 0; i < digits; ++i )
    {
        ...
        {
            if ( other.integer[j] == 0 ) continue;
            tmp[ i + j ] += integer[i] * other.integer[j];
        }
    }
    int carry = 0;
    ...
    HugeInt result;
    for ( int i = 0; i < digits; ++i )
        result.integer[i] = tmp[i];
    return result;
}

HugeInt HugeInt::operator/( const HugeInt & other ) const
{
    if ( other == HugeInt( 0L ) ) return HugeInt( 0L );
    if ( *this < other ) return HugeInt( 0L );
    HugeInt quotient, remainder;
    int len = getLength();
    for ( int i = len - 1; i >= 0; --i )
    {
        remainder = remainder * 10 + (int)integer[i];
        int digit = 0;
