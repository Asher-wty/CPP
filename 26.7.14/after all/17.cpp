...
{
    if (Vfirst < virtualZero || Vlast > virtualZero + (int)numofItems || Vfirst > Vlast) {
        cout << endl << "Out Of Range";
        exit(1);
    }
    DynamicVector<T> result(Vfirst);
...
void DynamicVector<T>::swap(DynamicVector<T>& dv)
{
    ...
    mallocSize = dv.mallocSize;
    dv.mallocSize = tmpSize;

    unsigned tmpNum = numofItems;
    numofItems = dv.numofItems;
    dv.numofItems = tmpNum;

    int tmpZero = virtualZero;
    virtualZero = dv.virtualZero;
    dv.virtualZero = tmpZero;
}

template <class T>
T* DynamicVector<T>::begin() const
{
    return array;
}

template <class T>
T* DynamicVector<T>::end() const
{
